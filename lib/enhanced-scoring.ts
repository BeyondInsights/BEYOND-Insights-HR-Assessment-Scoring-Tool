...
function statusToPoints(status: string | number): { points: number | null; isUnsure: boolean } {
  ...
  if (typeof status === 'string') {
    ...
    if (s === 'in place') return { points: GRID_POINTS.CURRENTLY_OFFER, isUnsure: false };
    if (s === 'in development') return { points: GRID_POINTS.PLANNING, isUnsure: false };
    if (s === 'under review') return { points: GRID_POINTS.ASSESSING, isUnsure: false };
    if (s === 'open to exploring') return { points: GRID_POINTS.ASSESSING, isUnsure: false };
    if (s === 'not planned') return { points: GRID_POINTS.NOT_ABLE, isUnsure: false };
    if (s === 'unsure' || s.includes('unsure') || s.includes('unknown')) return { points: null, isUnsure: true };
    // 2026 scale (backward compat — fuzzy matching)
