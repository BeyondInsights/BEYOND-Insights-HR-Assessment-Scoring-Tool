...
/* ================== Short descriptor overrides ==================
   Anything not listed falls back to an auto-shortener (no full questions). */
const SHORT: Record<string, string> = {
  ...
  // Examples — extend freely for your IDs (kept minimal here so this file works out of the box)
...
/* Shorten question text into a descriptor (when not in SHORT map) */
...
function descriptorFor(it:InstrumentItem){
  return SHORT[it.id] || autoShort(it.text) || it.id;
}

/* ================== The row (Descriptor: Answer) ================== */
function Row({ label, value, color }:{
  ...
  const display =
    value===undefined || value===null || value==='' ? '—'
      : Array.isArray(value) ? value.filter(v=>v!=='' && v!==null).join(' • ')
      : typeof value==='object' ? null
      : String(value);

  return (
    <div className="flex items-start gap-4 py-1.5 border-b last:border-b-0"
         style={{borderColor:BRAND.gray[200]}}>
      <div className="label-col flex items-center gap-2 shrink-0"
           style={{width:'18rem', maxWidth:'45%', minWidth:'16rem'}}>
        <Accent color={color}/>
        <span className="text-[13px] font-semibold" style={{color:BRAND.gray[600]}}>
          {label}:
        </span>
      </div>
      <div className="value-col text-[13px] text-right grow" style={{color:BRAND.gray[900]}}>
        {display!==null ? (
          looksScale(display)
...
export default function CompanyProfile(){
  ...
  useEffect(()=>{
    ...
    for (let i=1;i<=13;i++){
      ...
      d.push({ number:i, data: raw || {} }); // include all dims (we show EVERYTHING)
    }
      ...
      <div className="bg-white border-b" style={{borderColor:BRAND.gray[200]}}>
        <div className="max-w-7xl mx-auto px-6 py-6">
          ...
          <div className="text-center">
            <h1 className="text-4xl sm:text-5xl font-black mb-1" style={{color:BRAND.purple.primary}}>{companyName}</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <div className="border rounded-xl bg-white p-4" style={{borderColor:BRAND.gray[200]}}>
            ...
            <div className="space-y-1">
              ...
              <div className="text-sm" style={{color:BRAND.gray[900]}}>{firmo?.contactTitle || firmo?.hr_title || firmo?.title || '—'}</div>
              <div className="text-sm" style={{color:BRAND.gray[900]}}>{firmo?.s3 || firmo?.department || '—'}</div>
              <div className="text-sm" style={{color:BRAND.gray[900]}}>{firmo?.contactEmail || firmo?.hr_email || accountEmail || '—'}</div>
              <div className="text-sm" style={{color:BRAND.gray[900]}}>{firmo?.contactPhone || firmo?.hr_phone || firmo?.phone || '—'}</div>
              <div className="text-sm" style={{color:BRAND.gray[900]}}>{firmo?.hq || firmo?.s9 || '—'}</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <main className="max-w-7xl mx-auto px-6 mt-6">
        ...
        <Section title="Company Profile (Firmographics & Classification)" tone={BRAND.purple}>
          <div className="divide-y" style={{borderColor:BRAND.gray[200]}}>
            {splitEven(firmoItems)[0].map(it=>(
              <Row key={it.id} label={descriptorFor(it)} value={valueFrom(firmo,it)} color={BRAND.purple.primary}/>
            ))}
          </div>
          <div className="divide-y" style={{borderColor:BRAND.gray[200]}}>
            {splitEven(firmoItems)[1].map(it=>(
              <Row key={it.id} label={descriptorFor(it)} value={valueFrom(firmo,it)} color={BRAND.purple.primary}/>
            ))}
          </div>
        </Section>
        ...
        <Section title="General Employee Benefits" tone={BRAND.teal}>
          <div className="divide-y" style={{borderColor:BRAND.gray[200]}}>
            {splitEven(generalItems)[0].map(it=>(
              <Row key={it.id} label={descriptorFor(it)} value={valueFrom(general,it)} color={BRAND.teal.primary}/>
            ))}
          </div>
          <div className="divide-y" style={{borderColor:BRAND.gray[200]}}>
            {splitEven(generalItems)[1].map(it=>(
              <Row key={it.id} label={descriptorFor(it)} value={valueFrom(general,it)} color={BRAND.teal.primary}/>
            ))}
          </div>
        </Section>
        ...
        <Section title="Current Support for Employees Managing Cancer" tone={BRAND.orange}>
          <div className="divide-y" style={{borderColor:BRAND.gray[200]}}>
            {splitEven(currentItems)[0].map(it=>(
              <Row key={it.id} label={descriptorFor(it)} value={valueFrom(current,it)} color={BRAND.orange.primary}/>
            ))}
          </div>
          <div className="divide-y" style={{borderColor:BRAND.gray[200]}}>
            {splitEven(currentItems)[1].map(it=>(
              <Row key={it.id} label={descriptorFor(it)} value={valueFrom(current,it)} color={BRAND.orange.primary}/>
            ))}
          </div>
        </Section>
        ...
        <Section title="Cross-Dimensional Assessment" tone={BRAND.purple}>
          <div className="divide-y" style={{borderColor:BRAND.gray[200]}}>
            {splitEven(crossItems)[0].map(it=>(
              <Row key={it.id} label={descriptorFor(it)} value={valueFrom(cross,it)} color={BRAND.purple.primary}/>
            ))}
          </div>
          <div className="divide-y" style={{borderColor:BRAND.gray[200]}}>
            {splitEven(crossItems)[1].map(it=>(
              <Row key={it.id} label={descriptorFor(it)} value={valueFrom(cross,it)} color={BRAND.purple.primary}/>
            ))}
          </div>
        </Section>
        ...
        <Section title="Employee Impact Assessment" tone={BRAND.orange}>
          <div className="divide-y" style={{borderColor:BRAND.gray[200]}}>
            {splitEven(impactItems)[0].map(it=>(
              <Row key={it.id} label={descriptorFor(it)} value={valueFrom(impact,it)} color={BRAND.orange.primary}/>
            ))}
          </div>
          <div className="divide-y" style={{borderColor:BRAND.gray[200]}}>
            {splitEven(impactItems)[1].map(it=>(
              <Row key={it.id} label={descriptorFor(it)} value={valueFrom(impact,it)} color={BRAND.orange.primary}/>
            ))}
          </div>
        </Section>
        ...
        {dims.map(({number, data})=>{
          ...
          return (
            <Section key={number} title={`Dimension ${number}: ${DIM_NAME[number]}`} tone={BRAND.purple}
              ...
              <div className="divide-y" style={{borderColor:BRAND.gray[200]}}>
                {dL.map(it=>(
                  <Row key={it.id} label={descriptorFor(it)} value={valueFrom(data,it)} color={BRAND.purple.primary}/>
                ))}
              </div>
              <div className="divide-y" style={{borderColor:BRAND.gray[200]}}>
                {dR.map(it=>(
