...
export default function Dimension13Page() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!data.d13a || Object.keys(data.d13a || {}).length === 0) {
        setStep(1);
        setCurrentItemIndex(0);
      } else {
        const gridItems = Object.keys(data.d13a || {});
        const firstIncomplete = gridItems.findIndex(item => !data.d13a[item]);
        
        if (firstIncomplete !== -1) {
          setStep(1);
          setCurrentItemIndex(firstIncomplete);
        } else if (!data.d13b || data.d13b.trim() === '') {
          setStep(2);
        } else if (checkIsMultiCountry && !data.d13aa) {
          setStep(3);
        }
  ...
  const setStatus = (item: string, status: string) => {
    ...
    setTimeout(() => {
      const nextUnansweredIndex = D13A_ITEMS.findIndex((itm, idx) => 
        idx > currentItemIndex && !ans.d13a?.[itm]
      );
      
      if (nextUnansweredIndex !== -1) {
        setCurrentItemIndex(nextUnansweredIndex);
      } else if (currentItemIndex < D13A_ITEMS.length - 1) {
  ...
  const validateStep = () => {
    switch(step) {
      case 1:
        const answeredCount = Object.keys(ans.d13a || {}).length;
        if (answeredCount < D13A_ITEMS.length) 
          return `Please evaluate all ${D13A_ITEMS.length} items (${answeredCount} completed)`;
        return null;
      
      case 2:
        if (showD13aa && !ans.d13aa) {
      ...
      case 4:
        if (!ans.d13_1) {
          return "Please select a communication frequency";
        }
        return null;
      ...
      <main className="flex-1 max-w-5xl mx-auto px-4 py-8">
          ...
          <div className="bg-white rounded-xl shadow-sm p-8">
            <div className="max-w-3xl mx-auto">
              <p className="text-lg text-gray-700 mb-6">
                Here is the final aspect of <strong>support programs</strong> for{" "}
              ...
              <div className="bg-gradient-to-r from-blue-600 to-blue-700 p-6 rounded-xl mb-8">
                <h2 className="text-3xl font-bold text-white mb-3">COMMUNICATION & AWARENESS</h2>
                <p className="text-blue-100 text-lg">
                  How organizations inform, educate, and engage employees about available workplace support programs for those managing cancer or other serious health conditions.
                </p>
              </div>
              ...
              <div className="bg-gray-50 p-6 rounded-lg border border-gray-200">
                ...
                <ul className="space-y-3 text-gray-700">
                  <li className="flex items-start">
                    ...
                    <span>You'll see different communication approaches associated with this dimension, one at a time</span>
                  </li>
                  <li className="flex items-start">
                    <span className="text-blue-600 mr-2 mt-1">•</span>
                    <span><strong>Indicate the current status of each approach within your organization</strong></span>
                  </li>
                  <li className="flex items-start">
                    <span className="text-blue-600 mr-2 mt-1">•</span>
                    <span>After selecting a response, it will automatically advance to the next approach</span>
                  </li>
                  <li className="flex items-start">
                    <span className="text-blue-600 mr-2 mt-1">•</span>
                  ...
                  <li className="flex items-start">
                    <span className="text-blue-600 mr-2 mt-1">•</span>
                    <span>Once all approaches are evaluated, the Continue button will appear</span>
                  </li>
                </ul>
              </div>
                ...
                >
                  ...
                  <div className="space-y-2">
                    {STATUS_OPTIONS.map((status) => (
                      <button
                        key={status}
                        onClick={() => setStatus(D13A_ITEMS[currentItemIndex], status)}
                        disabled={isTransitioning}
                        className={`w-full p-4 text-left rounded-lg border-2 transition-all transform ${
                          ...
                          ans.d13a?.[D13A_ITEMS[currentItemIndex]] === status
        ...
        {step === 3 && (
          <div className="bg-white p-6 rounded-lg shadow-sm">
            ...
            <p className="font-bold text-gray-900 mb-4">
              What other <span className="text-blue-600 font-bold">communication or awareness approaches</span> does your organization use that weren't listed?
            </p>
            <p className="text-sm text-gray-600 mb-4">(Please be as specific and detailed as possible)</p>
            
            <textarea
              value={ans.d13b || ""}
              onChange={(e) => setField("d13b", e.target.value)}
              className="w-full min-h-[120px] px-4 py-3 border-2 border-gray-300 rounded-lg"
              placeholder="Describe any additional approaches..."
            />
            
            <label className="flex items-center mt-3">
              <input
                type="checkbox"
                checked={ans.d13b_none || false}
                onChange={(e) => {
                  setField("d13b_none", e.target.checked);
                  if (e.target.checked) setField("d13b", "");
                }}
                className="w-4 h-4 mr-2"
              />
              <span className="text-sm">No other approaches</span>
            </label>
          </div>
        )}
