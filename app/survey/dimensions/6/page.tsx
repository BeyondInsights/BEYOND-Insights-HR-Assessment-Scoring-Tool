...
export default function Dimension6Page() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!data.d6a || Object.keys(data.d6a || {}).length === 0) {
        setStep(1);
        setCurrentItemIndex(0);
      } else {
        const gridItems = Object.keys(data.d6a || {});
        const firstIncomplete = gridItems.findIndex(item => !data.d6a[item]);
        
        if (firstIncomplete !== -1) {
          setStep(1);
          setCurrentItemIndex(firstIncomplete);
        } else if (!data.d6b || data.d6b.trim() === '') {
          setStep(2);
        } else if (checkIsMultiCountry && !data.d6aa) {
          setStep(3);
        }
