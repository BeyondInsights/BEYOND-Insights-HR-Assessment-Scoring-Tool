...
export default function Dimension6Page() {
  ...
  // Use the progressive status grid hook for d6a items
  const {
    items: D6A_ITEMS,
    currentItemIndex,
    isTransitioning,
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      if (grid && typeof grid === 'object' && Object.keys(grid).length > 0) {
        setStep(1);
        if (Object.keys(grid).length >= Math.ceil(D6A_ITEMS_BASE.length / 2)) {
          setViewMode('summary');
        }
      }
    }
  ...
  const hasExistingGridData = ans['d6a'] && typeof ans['d6a'] === 'object' && Object.keys(ans['d6a']).length > 0;
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold text-blue-900">
                      Progress: {Object.keys(ans.d6a || {}).length} of {D6A_ITEMS.length} elements rated
                    </p>
                    <p className="text-xs text-blue-700 mt-1">
                      {Object.keys(ans.d6a || {}).length === D6A_ITEMS.length 
                        ? '✓ All elements completed!' 
                        : `${D6A_ITEMS.length - Object.keys(ans.d6a || {}).length} elements remaining`}
                    </p>
                  </div>
