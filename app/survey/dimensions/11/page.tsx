...
export default function Dimension11Page() {
  ...
  useEffect(() => {
    const saved = ctx.getSectionData('dimension11');
    if (saved) {
      setAns(saved);
...
const showD11_1 = ans.d11a?.["At least 70% coverage for regionally / locally recommended screenings"] === "In Place";
...
const next = async () => {
  ...
  if (step === 1) {
    ...
    ctx.setSectionComplete('dimension11', true);
    await ctx.saveToSupabase('dimension11');
  ...
  const hasExistingGridData = ans['d11a'] && typeof ans['d11a'] === 'object' && Object.keys(ans['d11a']).length > 0;
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-gray-600">
              Dimension 11: Prevention, Wellness & Legal Compliance</span>
          </div>
            ...
            <div className="p-8">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold text-blue-900">
                      Progress: {Object.keys(ans.d11a || {}).length} of {D11A_ITEMS.length} elements rated
                    </p>
                    <p className="text-xs text-blue-700 mt-1">
                      {Object.keys(ans.d11a || {}).length === D11A_ITEMS.length 
                        ? '✓ All elements completed!' 
                        : `${D11A_ITEMS.length - Object.keys(ans.d11a || {}).length} elements remaining`}
                    </p>
                  </div>
              ...
              <div className="mb-6">
                ...
                >
                  ...
                  {ELEMENT_TOOLTIPS[D11A_ITEMS[currentItemIndex]] && (
                    <div className="flex items-start gap-2 bg-blue-50 border border-blue-200 rounded-lg px-3 py-2 mb-4 -mt-2">
                      ...
                      <p className="text-xs text-blue-700">{ELEMENT_TOOLTIPS[D11A_ITEMS[currentItemIndex]]}</p>
                    </div>
                  )}
        ...
        {step === 5 && (
          <div className="bg-white p-8 rounded-lg shadow-sm text-center">
            ...
            {(() => {
              let done = 1;
              for (let i = 1; i <= 13; i++) {
                if (i !== 11 && ctx.getSectionComplete(`dimension${i}`)) done++;
              }
              return (
