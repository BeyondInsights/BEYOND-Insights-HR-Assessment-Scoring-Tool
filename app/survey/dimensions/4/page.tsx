...
export default function Dimension4Page() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      if (grid && typeof grid === 'object' && Object.keys(grid).length > 0) {
        setStep(1);
        if (Object.keys(grid).length >= Math.ceil(D4A_ITEMS_BASE.length / 2)) {
          setViewMode('summary');
        }
      }
    }
  ...
  const showD4_1 = ans.d4a?.["Dedicated navigation support to help employees understand benefits and access medical care"] === "In Place";
  ...
  const next = async () => {
    ...
    if (step === 1) {
      ...
      ctx.setSectionComplete('dimension4', true);
      await ctx.saveToSupabase('dimension4');
      ...
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white flex flex-col">
        ...
        <main className="max-w-4xl mx-auto px-6 py-8 flex-1">
          <DimensionSummaryView
            dimensionNumber={4}
            dimensionName="Cancer Support Resources"
            gridData={ans['d4a'] || {}}
            allAnswers={ans}
            ...
            <div className="p-8">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold text-blue-900">
                      Progress: {Object.keys(ans.d4a || {}).length} of {D4A_ITEMS.length} elements rated
                    </p>
                    <p className="text-xs text-blue-700 mt-1">
                      {Object.keys(ans.d4a || {}).length === D4A_ITEMS.length 
                        ? '✓ All elements completed!' 
                        : `${D4A_ITEMS.length - Object.keys(ans.d4a || {}).length} elements remaining`}
                    </p>
                  </div>
              ...
              <div className="mb-6">
                ...
                >
                  ...
                  {ELEMENT_TOOLTIPS[D4A_ITEMS[currentItemIndex]] && (
                    <div className="flex items-start gap-2 bg-blue-50 border border-blue-200 rounded-lg px-3 py-2 mb-4 -mt-2">
                      ...
                      <p className="text-xs text-blue-700">{ELEMENT_TOOLTIPS[D4A_ITEMS[currentItemIndex]]}</p>
                    </div>
                  )}
        ...
        {step === 6 && (
          <div className="bg-white p-8 rounded-lg shadow-sm text-center">
            ...
            {(() => {
              let done = 1;
              for (let i = 1; i <= 13; i++) {
                if (i !== 4 && ctx.getSectionComplete(`dimension${i}`)) done++;
              }
              return (
