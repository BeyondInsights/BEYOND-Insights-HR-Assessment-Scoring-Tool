...
export default function Dimension9Page() {
  ...
  useEffect(() => {
    const saved = ctx.getSectionData('dimension9');
    if (saved) {
      setAns(saved);
      // If main grid has answers, skip intro and go to grid
      const grid = saved['d9a'];
      if (grid && typeof grid === 'object' && Object.keys(grid).length > 0) {
        setStep(1);
        if (Object.keys(grid).length >= Math.ceil(D9A_ITEMS_BASE.length / 2)) {
          setViewMode('summary');
        }
      }
    }
  ...
  useEffect(() => {
    if (Object.keys(ans).length > 0) {
      ctx.setSectionData('dimension9', ans);
    }
  }, [ans]);
...
const getTotalSteps = () => {
  ...
  const next = async () => {
    ...
    if (step === 1) {
      ...
      ctx.setSectionComplete('dimension9', true);
      await ctx.saveToSupabase('dimension9');
      ...
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white flex flex-col">
        ...
        <main className="max-w-4xl mx-auto px-6 py-8 flex-1">
          <DimensionSummaryView
            dimensionNumber={9}
            dimensionName="Executive Commitment & Resources"
            gridData={ans['d9a'] || {}}
            allAnswers={ans}
            ...
            onGridChange={(element, status) => {
              setAns((prev: any) => ({
                ...prev,
                d9a: { ...prev.d9a, [element]: status }
              }));
            }}
            onSwitchToStepView={() => { setViewMode('step'); setStep(2); }}
            onSave={async () => {
              ctx.setSectionData('dimension9', ans);
              return ctx.saveToSupabase('dimension9');
            }}
          />
        </main>
        <Footer />
            ...
            <div className="p-8">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold text-blue-900">
                      Progress: {Object.keys(ans.d9a || {}).length} of {D9A_ITEMS.length} elements rated
                    </p>
                    <p className="text-xs text-blue-700 mt-1">
                      {Object.keys(ans.d9a || {}).length === D9A_ITEMS.length 
                        ? '✓ All elements completed!' 
                        : `${D9A_ITEMS.length - Object.keys(ans.d9a || {}).length} elements remaining`}
                    </p>
                  </div>
              ...
              <div className="mb-6">
                ...
                >
                  ...
                  {ELEMENT_TOOLTIPS[D9A_ITEMS[currentItemIndex]] && (
                    <div className="flex items-start gap-2 bg-blue-50 border border-blue-200 rounded-lg px-3 py-2 mb-4 -mt-2">
                      ...
                      <p className="text-xs text-blue-700">{ELEMENT_TOOLTIPS[D9A_ITEMS[currentItemIndex]]}</p>
                    </div>
                  )}
        ...
        {step === 4 && (
          <div className="bg-white p-8 rounded-lg shadow-sm text-center">
            ...
            {(() => {
              let done = 1;
              for (let i = 1; i <= 13; i++) {
                if (i !== 9 && ctx.getSectionComplete(`dimension${i}`)) done++;
              }
              return (
