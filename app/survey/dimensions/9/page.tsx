...
export default function Dimension9Page() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!data.d9a || Object.keys(data.d9a || {}).length === 0) {
        setStep(1);
        setCurrentItemIndex(0);
      } else {
        const gridItems = Object.keys(data.d9a || {});
        const firstIncomplete = gridItems.findIndex(item => !data.d9a[item]);
        
        if (firstIncomplete !== -1) {
          setStep(1);
          setCurrentItemIndex(firstIncomplete);
        } else if (!data.d9b || data.d9b.trim() === '') {
          setStep(2);
        } else if (checkIsMultiCountry && !data.d9aa) {
          setStep(3);
        }
