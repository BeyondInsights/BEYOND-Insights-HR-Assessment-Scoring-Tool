...
export default function Dimension1Page() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      if (grid && typeof grid === 'object' && Object.keys(grid).length > 0) {
        setStep(1);
        if (Object.keys(grid).length >= Math.ceil(D1A_ITEMS_BASE.length / 2)) {
          setViewMode('summary');
        }
      }
    }
  ...
  const showD1_1 = ans.d1a?.["Paid medical leave beyond local / legal requirements"] === "In Place";
  const showD1_2 = ans.d1a?.["Intermittent leave beyond local / legal requirements"] === "In Place";
  const showD1_4a = ans.d1a?.["Remote work options for on-site employees"] === "In Place";
  ...
  const showD1_7 = ans.d1a?.["Full salary and health insurance continuation, beyond legal requirements"] === "In Place";
  ...
  const hasExistingGridData = ans['d1a'] && typeof ans['d1a'] === 'object' && Object.keys(ans['d1a']).length > 0;
            ...
            <div className="p-8">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold text-blue-900">
                      Progress: {Object.keys(ans.d1a || {}).length} of {D1A_ITEMS.length} elements rated
                    </p>
                    <p className="text-xs text-blue-700 mt-1">
                      {Object.keys(ans.d1a || {}).length === D1A_ITEMS.length 
                        ? '✓ All elements completed!' 
                        : `${D1A_ITEMS.length - Object.keys(ans.d1a || {}).length} elements remaining`}
                    </p>
                  </div>
              ...
              <div className="mb-6">
                ...
                >
                  ...
                  {ELEMENT_TOOLTIPS[D1A_ITEMS[currentItemIndex]] && (
                    <div className="flex items-start gap-2 bg-blue-50 border border-blue-200 rounded-lg px-3 py-2 mb-4 -mt-2">
                      ...
                      <p className="text-xs text-blue-700">{ELEMENT_TOOLTIPS[D1A_ITEMS[currentItemIndex]]}</p>
                    </div>
                  )}
  ...
  <div className="bg-white p-6 rounded-lg shadow-sm">
    ...
    <div className="space-y-2">
      {[
        ...
        {step === 10 && showD1_7 && (
          <div className="bg-white p-6 rounded-lg shadow-sm">
            <h3 className="text-xl font-bold text-gray-900 mb-4">Full Salary & Health Insurance Continuation</h3>

            <p className="text-gray-700 mb-2">
              What is the total duration of the <strong>guaranteed salary and health insurance continuation</strong>?
            </p>
        ...
        {step === 11 && (
          <div className="bg-white p-8 rounded-lg shadow-sm text-center">
            ...
            {(() => {
              let done = 1;
              for (let i = 1; i <= 13; i++) {
                if (i !== 1 && ctx.getSectionComplete(`dimension${i}`)) done++;
              }
              return (
