...
export default function Dimension5Page() {
  ...
  // Use the progressive status grid hook for d5a items
  const {
    items: D5A_ITEMS,
    currentItemIndex,
    isTransitioning,
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      const grid = saved['d5a'];
      if (grid && typeof grid === 'object' && Object.keys(grid).length > 0) {
        setStep(1);
        if (Object.keys(grid).length >= Math.ceil(D5A_ITEMS_BASE.length / 2)) {
          setViewMode('summary');
        }
      }
    }
  ...
  const validateStep = () => {
    switch(step) {
      case 1:
        if (answeredCount < D5A_ITEMS.length) 
          return `Please evaluate all ${D5A_ITEMS.length} items (${answeredCount} completed)`;
        return null;
      ...
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white flex flex-col">
        ...
        <main className="max-w-4xl mx-auto px-6 py-8 flex-1">
          <DimensionSummaryView
            dimensionNumber={5}
            dimensionName="Workplace Accommodations"
            gridData={ans['d5a'] || {}}
            allAnswers={ans}
            ...
            onGridChange={(element, status) => {
              setAns((prev: any) => ({
                ...prev,
                d5a: { ...prev.d5a, [element]: status }
              }));
            }}
            onSwitchToStepView={() => { setViewMode('step'); setStep(2); }}
            onSave={async () => {
              ctx.setSectionData('dimension5', ans);
              return ctx.saveToSupabase('dimension5');
            }}
          />
        </main>
        <Footer />
      ...
      <main className="flex-1 max-w-5xl mx-auto px-4 py-8">
        <div className="mb-6">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-gray-600">
              Dimension 5: Workplace Accommodations & Modifications</span>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-8">
            <div className="max-w-3xl mx-auto">
              ...
              <div className="bg-gradient-to-r from-blue-600 to-blue-700 p-6 rounded-xl mb-8">
                <h2 className="text-3xl font-bold text-white mb-3">WORKPLACE ACCOMMODATIONS & MODIFICATIONS</h2>
                <p className="text-blue-100 text-lg">
                  Workplace changes that enable continued productivity during and after treatment, including physical modifications, schedule adjustments, policy changes, and technological support.
                </p>
              </div>
            ...
            <div className="bg-gradient-to-r from-gray-900 to-gray-800 px-8 py-6 rounded-t-xl">
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  <h2 className="text-2xl font-bold text-white mb-2">WORKPLACE ACCOMMODATIONS & MODIFICATIONS</h2>
                  <p className="text-gray-300 text-sm">
                    Workplace changes that enable continued productivity during and after treatment
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="p-8">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-2xl font-bold text-blue-900">
                    {Math.round((Object.keys(ans.d5a || {}).length / D5A_ITEMS.length) * 100)}%
                  </div>
                </div>
                <div className="mt-3 w-full bg-blue-200 rounded-full h-2">
                  <div 
                    className="bg-blue-600 h-2 rounded-full transition-all"
                    style={{ width: `${(Object.keys(ans.d5a || {}).length / D5A_ITEMS.length) * 100}%` }}
                  />
                </div>
              </div>
              ...
              <div className="mb-6">
                ...
                >
                  ...
                  {ELEMENT_TOOLTIPS[D5A_ITEMS[currentItemIndex]] && (
                    <div className="flex items-start gap-2 bg-blue-50 border border-blue-200 rounded-lg px-3 py-2 mb-4 -mt-2">
                      ...
                      <p className="text-xs text-blue-700">{ELEMENT_TOOLTIPS[D5A_ITEMS[currentItemIndex]]}</p>
                    </div>
                  )}
                  ...
                  <div className="space-y-2">
                    {STATUS_OPTIONS.map((status) => (
                      <button
                        key={status}
                        onClick={() => setStatus(D5A_ITEMS[currentItemIndex], status)}
                        disabled={isTransitioning}
                        className={`w-full p-4 text-left rounded-lg border-2 transition-all transform ${
                          ...
                          ans.d5a?.[D5A_ITEMS[currentItemIndex]] === status
        ...
        {step === 2 && showd5aa && (
          <div className="bg-white p-6 rounded-lg shadow-sm">
            ...
            <div className={`border-2 rounded-lg p-4 ${
              touched.d5aa && !ans.d5aa
                ? 'border-red-500 bg-red-50' 
                : 'border-gray-200 bg-white'
            }`}>
              {/* VALIDATION: Required asterisk */}
              <p className="font-bold text-gray-900 mb-4">
                Are the <span className="text-blue-600 font-bold">Workplace Accommodations & Modifications</span> your 
              ...
              <div className="space-y-2">
                {[
                  ...
                  <button
                    ...
                    onClick={() => {
                      setField("d5aa", opt);
                      markTouched("d5aa"); // VALIDATION: Mark as touched
                    }}
        ...
        {step === 3 && (
          <div className="bg-white p-6 rounded-lg shadow-sm">
            ...
            <p className="font-bold text-gray-900 mb-4">
              What other <span className="text-blue-600 font-bold">workplace accommodations and modifications</span> does your organization offer that weren't listed?
            </p>
            <p className="text-sm text-gray-600 mb-4">(Please be as specific and detailed as possible)</p>
            
            <textarea
              value={ans.d5b || ""}
              onChange={(e) => setField("d5b", e.target.value)}
              className="w-full min-h-[120px] px-4 py-3 border-2 border-gray-300 rounded-lg"
              placeholder="Describe any additional accommodations..."
            />
            
            <label className="flex items-center mt-3">
              <input
                type="checkbox"
                checked={ans.d5b_none || false}
                onChange={(e) => {
                  setField("d5b_none", e.target.checked);
                  if (e.target.checked) setField("d5b", "");
                }}
                className="w-4 h-4 mr-2"
              />
              <span className="text-sm">No others</span>
            </label>
          </div>
        )}
        ...
        {step === 4 && (
          <div className="bg-white p-8 rounded-lg shadow-sm text-center">
            ...
            {(() => {
              let done = 1;
              for (let i = 1; i <= 13; i++) {
                if (i !== 5 && ctx.getSectionComplete(`dimension${i}`)) done++;
              }
              return (
