...
export default function Dimension5Page() {
  ...
  const setStatus = (item: string, status: string) => {
    setAns((prev: any) => ({
      ...prev,
      d5a: { ...(prev.d5a || {}), [item]: status }
    }));
    
    // VALIDATION: Mark d5a as touched
    markTouched('d5a');
    
    setIsTransitioning(true);
    
    setTimeout(() => {
      const nextUnansweredIndex = D5A_ITEMS.findIndex((itm, idx) => 
        idx > currentItemIndex && !ans.d5a?.[itm]
      );
      
      if (nextUnansweredIndex !== -1) {
        setCurrentItemIndex(nextUnansweredIndex);
      } else if (currentItemIndex < D5A_ITEMS.length - 1) {
  ...
  const validateStep = () => {
    switch(step) {
      case 1:
        const answeredCount = Object.keys(ans.d5a || {}).length;
