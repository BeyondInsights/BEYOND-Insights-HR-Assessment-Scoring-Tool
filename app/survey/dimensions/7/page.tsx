...
export default function Dimension7Page() {
  ...
  const [D7A_ITEMS] = useState(() => shuffleArray(D7A_ITEMS_BASE));
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!data.d7a || Object.keys(data.d7a || {}).length === 0) {
        setStep(1);
        setCurrentItemIndex(0);
      } else {
        const gridItems = Object.keys(data.d7a || {});
        const firstIncomplete = gridItems.findIndex(item => !data.d7a[item]);
        
        if (firstIncomplete !== -1) {
          setStep(1);
          setCurrentItemIndex(firstIncomplete);
        } else if (!data.d7b || data.d7b.trim() === '') {
          setStep(2);
        } else if (checkIsMultiCountry && !data.d7aa) {
          setStep(3);
        }
  ...
  const setStatus = (item: string, status: string) => {
    setAns((prev: any) => ({
      ...prev,
      d7a: { ...(prev.d7a || {}), [item]: status }
    }));
    
    markTouched('d7a'); // VALIDATION: Mark d7a as touched
    
    setIsTransitioning(true);
    
    setTimeout(() => {
      const nextUnansweredIndex = D7A_ITEMS.findIndex((itm, idx) => 
        idx > currentItemIndex && !ans.d7a?.[itm]
      );
      
      if (nextUnansweredIndex !== -1) {
        setCurrentItemIndex(nextUnansweredIndex);
      } else if (currentItemIndex < D7A_ITEMS.length - 1) {
  ...
  const validateStep = () => {
    switch(step) {
      case 1:
        const answeredCount = Object.keys(ans.d7a || {}).length;
        if (answeredCount < D7A_ITEMS.length) 
          return `Please evaluate all ${D7A_ITEMS.length} items (${answeredCount} completed)`;
        return null;
      ...
      <main className="flex-1 max-w-5xl mx-auto px-4 py-8">
          ...
          <div className="bg-white rounded-xl shadow-sm p-8">
            <div className="max-w-3xl mx-auto">
              ...
              <div className="bg-gradient-to-r from-blue-600 to-blue-700 p-6 rounded-xl mb-8">
                <h2 className="text-3xl font-bold text-white mb-3">CAREER CONTINUITY & ADVANCEMENT</h2>
                <p className="text-blue-100 text-lg">
                  Protections for career advancement opportunities and professional development during and after treatment.
                </p>
              </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold text-blue-900">
                      Progress: {Object.keys(ans.d7a || {}).length} of {D7A_ITEMS.length} items rated
                    </p>
                    <p className="text-xs text-blue-700 mt-1">
                      {Object.keys(ans.d7a || {}).length === D7A_ITEMS.length 
                        ? '✓ All items completed!' 
                        : `${D7A_ITEMS.length - Object.keys(ans.d7a || {}).length} items remaining`}
                    </p>
                  </div>
                  <div className="text-2xl font-bold text-blue-900">
                    {Math.round((Object.keys(ans.d7a || {}).length / D7A_ITEMS.length) * 100)}%
                  </div>
                </div>
                <div className="mt-3 w-full bg-blue-200 rounded-full h-2">
                  <div 
                    className="bg-blue-600 h-2 rounded-full transition-all"
                    style={{ width: `${(Object.keys(ans.d7a || {}).length / D7A_ITEMS.length) * 100}%` }}
                  />
                </div>
              </div>
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between">
                  <span className="text-lg font-bold text-gray-800">
                    {resumeComplete 
                      ? `Item ${currentItemIndex + 1} of ${D7A_ITEMS.length}`
                      : 'Loading position...'
                    }
                  </span>
                  <div className="flex gap-1">
                    {D7A_ITEMS.map((item, idx) => (
                      <button
                        key={idx}
                ...
                >
                  ...
                  <div className="space-y-2">
                    {STATUS_OPTIONS.map((status) => (
                      <button
                        key={status}
                        onClick={() => setStatus(D7A_ITEMS[currentItemIndex], status)}
                        disabled={isTransitioning}
                        className={`w-full p-4 text-left rounded-lg border-2 transition-all transform ${
                          ...
                          ans.d7a?.[D7A_ITEMS[currentItemIndex]] === status
              ...
              <div className="space-y-2">
                {[
                  ...
                  <button
                    ...
                    onClick={() => {
                      setField("d7aa", opt);
                      markTouched("d7aa"); // VALIDATION: Mark as touched
                    }}
        ...
        {(step === 3 || (step === 2 && !showD7aa)) && (
          <div className="bg-white p-6 rounded-lg shadow-sm">
            ...
            <p className="font-bold text-gray-900 mb-4">
              What other <span className="text-blue-600 font-bold">career continuity and advancement supports</span> does your organization offer employees managing cancer or other serious health conditions that weren't listed?
            </p>
            <p className="text-sm text-gray-600 mb-4">(Please be as specific and detailed as possible)</p>
            
            <textarea
              value={ans.d7b || ""}
              onChange={(e) => setField("d7b", e.target.value)}
            ...
            <label className="flex items-center mt-3">
              <input
                type="checkbox"
                checked={ans.d7b_none || false}
                onChange={(e) => {
                  setField("d7b_none", e.target.checked);
                  if (e.target.checked) setField("d7b", "");
                }}
        ...
        {step === 4 && (
          <div className="bg-white p-8 rounded-lg shadow-sm text-center">
            ...
            <p className="text-gray-600 mb-8">
              You've successfully completed the Career Continuity & Advancement dimension.
            </p>
            <button
              onClick={() => { 
                localStorage.setItem("dimension7_complete", "true"); 
