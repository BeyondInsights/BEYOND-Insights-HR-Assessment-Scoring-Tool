        ...
        <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4" onClick={() => setShowPdfOrientationModal(false)}>
          ...
          >
            <div className="bg-slate-800 px-6 py-4">
              ...
              <p className="text-slate-400 text-sm mt-1">Choose your preferred format</p>
            </div>
            ...
            <div className="p-6 space-y-4">
              <button
                onClick={() => {
                  ...
                  const url = `/.netlify/functions/export-pdf?surveyId=${encodeURIComponent(String(surveyId || ''))}&orientation=portrait`;
                  window.open(url, '_blank');
                }}
                className="w-full py-4 px-4 bg-white border-2 border-slate-200 hover:border-indigo-500 hover:bg-indigo-50 rounded-xl transition-all flex items-center gap-4"
              >
                <div className="w-12 h-16 bg-slate-100 rounded border-2 border-slate-300 flex items-center justify-center">
                  <svg className="w-6 h-8 text-slate-400" fill="currentColor" viewBox="0 0 24 32">
                    <rect x="2" y="2" width="20" height="28" rx="2" fill="none" stroke="currentColor" strokeWidth="2"/>
                  </svg>
                </div>
                <div className="text-left">
                  <p className="font-semibold text-slate-800">Portrait</p>
                  <p className="text-sm text-slate-500">Standard document format (8.5" × 11")</p>
                </div>
              </button>
              ...
              <button
                onClick={() => {
                  setShowPdfOrientationModal(false);
                  const url = `/.netlify/functions/export-pdf?surveyId=${encodeURIComponent(String(surveyId || ''))}&orientation=landscape`;
                  window.open(url, '_blank');
                }}
                className="w-full py-4 px-4 bg-white border-2 border-slate-200 hover:border-indigo-500 hover:bg-indigo-50 rounded-xl transition-all flex items-center gap-4"
              >
                <div className="w-16 h-12 bg-slate-100 rounded border-2 border-slate-300 flex items-center justify-center">
                  <svg className="w-8 h-6 text-slate-400" fill="currentColor" viewBox="0 0 32 24">
                    <rect x="2" y="2" width="28" height="20" rx="2" fill="none" stroke="currentColor" strokeWidth="2"/>
                  </svg>
                </div>
                <div className="text-left">
                  <p className="font-semibold text-slate-800">Landscape</p>
                  <p className="text-sm text-slate-500">Presentation slides format (16:9)</p>
                </div>
              </button>
