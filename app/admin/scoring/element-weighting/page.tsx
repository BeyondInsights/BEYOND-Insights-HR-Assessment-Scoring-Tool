...
interface CombinedCompanyResult {
  ...
  isComplete: boolean;
  isPanel: boolean;
  dims: Record<number, { oldEq: number; newEq: number; wt: number; unsure: number; combined: number }>;
  oldEqComposite: number;    // Old dim weights, equal element weights, unsure=0
  newEqComposite: number;    // New dim weights, equal element weights, unsure=0
  wtComposite: number;       // New dim weights, element-weighted, unsure=0
  unsureComposite: number;   // New dim weights, equal element weights + unsure substitution
  combinedComposite: number; // New dim weights, element-weighted + unsure substitution
  maturityScore: number;
  breadthScore: number;
}
...
function calculateCombinedScore(
  ...
  // Get baseline scores using NEW dimension weights
  const eqScores = calculateCompanyScores(assessment, DEFAULT_DIMENSION_WEIGHTS, DEFAULT_COMPOSITE_WEIGHTS, DEFAULT_BLEND_WEIGHTS);
  // Also compute with OLD dimension weights for comparison
  const oldEqScores = calculateCompanyScores(assessment, OLD_DIMENSION_WEIGHTS, DEFAULT_COMPOSITE_WEIGHTS, DEFAULT_BLEND_WEIGHTS);

  const dims: Record<number, { oldEq: number; newEq: number; wt: number; unsure: number; combined: number }> = {};

  for (let i = 1; i <= 13; i++) {
    const dimData = assessment[`dimension${i}_data`];

    // Old equal-weight score (from old dimension weights scoring - dimension scores are the same, composite differs)
    const oldEqScore = oldEqScores.dimensions[i]?.blendedScore || 0;
    // New equal-weight score (same dimension scores, but will use new dim weights for composite)
    const newEqScore = eqScores.dimensions[i]?.blendedScore || 0;
    ...
    dims[i] = { oldEq: oldEqScore, newEq: newEqScore, wt: wtScore, unsure: unsureScore, combined: combinedScore };
  }

  // Composites using NEW dimension weights
  const totalWeight = Object.values(DEFAULT_DIMENSION_WEIGHTS).reduce((s, w) => s + w, 0);
  const oldTotalWeight = Object.values(OLD_DIMENSION_WEIGHTS).reduce((s, w) => s + w, 0);

  let oldEqWeightedDim = 0;
  let newEqWeightedDim = 0;
  let wtWeightedDim = 0;
  ...
  if (eqScores.isComplete && totalWeight > 0) {
    for (let d = 1; d <= 13; d++) {
      ...
      const oldW = OLD_DIMENSION_WEIGHTS[d] || 0;
      oldEqWeightedDim += dims[d].oldEq * (oldW / oldTotalWeight);
      newEqWeightedDim += dims[d].newEq * (w / totalWeight);
  ...
  return {
    ...
    isPanel: eqScores.isPanel,
    dims,
    oldEqComposite: calcComposite(oldEqWeightedDim),
    newEqComposite: calcComposite(newEqWeightedDim),
...
// ============================================
// ELEMENT WEIGHTS — v6 (Source: Element_Weighting_Final.xlsx)
// Ordinal Ridge + Permutation Importance | 50/50 shrinkage | 20% cap
// D1 & D13 duplicates merged: higher weight + 50% of lower, renormalized
// Adaptive α shrinkage retained from v6.1 methodology
// ============================================

const ELEMENT_WEIGHTS: Record<number, Record<string, number>> = {
  1: {
    // 13→12 after merge: "Paid micro-breaks" duplicate removed; 50% of lower distributed to ranks 3-12
    'Emergency leave within 24 hours': 0.164705,
    'Remote work options for on-site employees': 0.125719,
    'Intermittent leave beyond local / legal requirements': 0.110911,
    'Paid micro-breaks for medical-related side effects': 0.093921,
    'Flexible work hours during treatment (e.g., varying start/end times, compressed schedules)': 0.093364,
    'Job protection beyond local / legal requirements': 0.070545,
    'Paid medical leave beyond local / legal requirements': 0.062391,
    'Reduced schedule/part-time with full benefits': 0.060976,
    'Disability pay top-up (employer adds to disability insurance)': 0.055834,
    'Full salary (100%) continuation during cancer-related short-term disability leave': 0.055732,
    'PTO accrual during leave': 0.053554,
    'Leave donation bank (employees can donate PTO to colleagues)': 0.052348,
  },
  2: {
    ...
    'Coverage for advanced therapies (CAR-T, proton therapy, immunotherapy) not covered by standard health insurance': 0.046940,
    ...
    'Employer-paid disability insurance supplements': 0.042410,
    'Guaranteed job protection': 0.040960,
  ...
  3: {
    ...
    'Dedicated manager resource hub': 0.099820,
  ...
  4: {
    ...
    'Survivorship planning assistance': 0.077310,
  ...
  6: {
    ...
    'Written anti-retaliation policies for health disclosures': 0.065130,
  ...
  7: {
    ...
    'Project continuity protocols': 0.077280,
  },
  8: {
    ...
    'Phased return-to-work plans': 0.059020,
  ...
  9: {
    ...
    'Executive accountability metrics': 0.057550,
  ...
  10: {
    ...
    'Respite care funding/reimbursement': 0.043040,
  ...
  11: {
    'Legal protections beyond requirements': 0.115400,
    ...
    'Risk factor tracking/reporting': 0.066010,
    ...
    'Lifestyle coaching programs': 0.051920,
  },
  12: {
  ...
  13: {
    // 11→10 after merge: "Anonymous info" duplicate removed; 50% of lower distributed to ranks 3,5,7-11
    'Family/caregiver communication inclusion': 0.183852,
    'Employee testimonials/success stories': 0.125551,
    'Proactive communication at point of diagnosis disclosure': 0.124916,
    'Multi-channel communication strategy': 0.114740,
    'Ability to access program information and resources anonymously': 0.112105,
    'Dedicated program website or portal': 0.080913,
    'New hire orientation coverage': 0.066129,
    'Regular company-wide awareness campaigns (at least quarterly)': 0.064832,
    'Manager toolkit for cascade communications': 0.064606,
    'Cancer awareness month campaigns with resources': 0.062357,
  },
};
...
const DIMENSIONS: Record<number, DimensionData> = {
  1: {
    ...
    items: [
      ...
      { rank: 8, name: "Reduced schedule/part-time with full benefits", weight: 0.060976, equal: 0.083333, delta: -0.022357 },
      { rank: 9, name: "Disability pay top-up (employer adds to disability insurance)", weight: 0.055834, equal: 0.083333, delta: -0.027499 },
      { rank: 10, name: "Full salary (100%) continuation during cancer-related STD leave", weight: 0.055732, equal: 0.083333, delta: -0.027601 },
      { rank: 11, name: "PTO accrual during leave", weight: 0.053554, equal: 0.083333, delta: -0.029779 },
      { rank: 12, name: "Leave donation bank (employees can donate PTO to colleagues)", weight: 0.052348, equal: 0.083333, delta: -0.030985 },
    ]
  },
  2: {
    ...
    items: [
      { rank: 1, name: "Accelerated life insurance benefits (partial payout for terminal / ...", weight: 0.154645, equal: 0.058824, delta: 0.095821 },
      { rank: 2, name: "Tax/estate planning assistance", weight: 0.118162, equal: 0.058824, delta: 0.059339 },
      { rank: 3, name: "Real-time cost estimator tools", weight: 0.073349, equal: 0.058824, delta: 0.014526 },
      { rank: 4, name: "Insurance advocacy/pre-authorization support", weight: 0.072306, equal: 0.058824, delta: 0.013483 },
      { rank: 5, name: "$0 copay for specialty drugs", weight: 0.056197, equal: 0.058824, delta: -0.002626 },
      { rank: 6, name: "Short-term disability covering 60%+ of salary", weight: 0.055313, equal: 0.058824, delta: -0.003510 },
      { rank: 7, name: "Long-term disability covering 60%+ of salary", weight: 0.048299, equal: 0.058824, delta: -0.010525 },
      { rank: 8, name: "Coverage for advanced therapies (CAR-T, proton therapy, immunothera...", weight: 0.046940, equal: 0.058824, delta: -0.011884 },
      { rank: 9, name: "Financial counseling services", weight: 0.046897, equal: 0.058824, delta: -0.011926 },
      { rank: 10, name: "Paid time off for clinical trial participation", weight: 0.043348, equal: 0.058824, delta: -0.015476 },
      { rank: 11, name: "Coverage for clinical trials and experimental treatments not covere...", weight: 0.043113, equal: 0.058824, delta: -0.015710 },
      { rank: 12, name: "Employer-paid disability insurance supplements", weight: 0.042410, equal: 0.058824, delta: -0.016413 },
      { rank: 13, name: "Guaranteed job protection", weight: 0.040960, equal: 0.058824, delta: -0.017863 },
      { rank: 14, name: "Travel/lodging reimbursement for specialized care beyond insurance ...", weight: 0.039906, equal: 0.058824, delta: -0.018917 },
      { rank: 15, name: "Hardship grants program funded by employer", weight: 0.039768, equal: 0.058824, delta: -0.019056 },
      { rank: 16, name: "Voluntary supplemental illness insurance (with employer contribution)", weight: 0.039559, equal: 0.058824, delta: -0.019264 },
      { rank: 17, name: "Set out-of-pocket maximums (for in-network single coverage)", weight: 0.038828, equal: 0.058824, delta: -0.019996 },
    ]
  },
  3: {
    ...
    items: [
      { rank: 1, name: "Manager peer support / community building", weight: 0.174725, equal: 0.1, delta: 0.074725 },
      { rank: 2, name: "Manager training on supporting employees managing cancer or other s...", weight: 0.154583, equal: 0.1, delta: 0.054583 },
      { rank: 3, name: "Empathy/communication skills training", weight: 0.140133, equal: 0.1, delta: 0.040133 },
      { rank: 4, name: "Dedicated manager resource hub", weight: 0.099820, equal: 0.1, delta: -0.000180 },
      { rank: 5, name: "Manager evaluations include how well they support impacted employees", weight: 0.080495, equal: 0.1, delta: -0.019505 },
      { rank: 6, name: "Clear escalation protocol for manager response", weight: 0.077347, equal: 0.1, delta: -0.022653 },
      { rank: 7, name: "Legal compliance training", weight: 0.071002, equal: 0.1, delta: -0.028998 },
      { rank: 8, name: "AI-powered guidance tools", weight: 0.069294, equal: 0.1, delta: -0.030706 },
      { rank: 9, name: "Privacy protection and confidentiality management", weight: 0.067211, equal: 0.1, delta: -0.032789 },
      { rank: 10, name: "Senior leader coaching on supporting impacted employees", weight: 0.065389, equal: 0.1, delta: -0.034611 },
    ]
  },
  4: {
    ...
    items: [
      { rank: 1, name: "Physical rehabilitation support", weight: 0.200073, equal: 0.1, delta: 0.100073 },
      { rank: 2, name: "Nutrition coaching", weight: 0.133196, equal: 0.1, delta: 0.033196 },
      { rank: 3, name: "Insurance advocacy/appeals support", weight: 0.102184, equal: 0.1, delta: 0.002184 },
      { rank: 4, name: "Dedicated navigation support to help employees understand benefits ...", weight: 0.089426, equal: 0.1, delta: -0.010574 },
      { rank: 5, name: "Online tools, apps, or portals for health/benefits support", weight: 0.087604, equal: 0.1, delta: -0.012396 },
      { rank: 6, name: "Occupational therapy/vocational rehabilitation", weight: 0.081315, equal: 0.1, delta: -0.018685 },
      { rank: 7, name: "Care coordination concierge", weight: 0.078595, equal: 0.1, delta: -0.021405 },
      { rank: 8, name: "Survivorship planning assistance", weight: 0.077310, equal: 0.1, delta: -0.022690 },
      { rank: 9, name: "Benefits optimization assistance (maximizing coverage, minimizing c...", weight: 0.076675, equal: 0.1, delta: -0.023325 },
      { rank: 10, name: "Clinical trial matching service", weight: 0.073622, equal: 0.1, delta: -0.026378 },
    ]
  },
  5: {
    ...
    items: [
      { rank: 1, name: "Flexible scheduling options", weight: 0.134067, equal: 0.090909, delta: 0.043158 },
      { rank: 2, name: "Ergonomic equipment funding", weight: 0.126183, equal: 0.090909, delta: 0.035274 },
      { rank: 3, name: "Rest areas / quiet spaces", weight: 0.115475, equal: 0.090909, delta: 0.024566 },
      { rank: 4, name: "Temporary role redesigns", weight: 0.109129, equal: 0.090909, delta: 0.018220 },
      { rank: 5, name: "Assistive technology catalog", weight: 0.108836, equal: 0.090909, delta: 0.017927 },
      { rank: 6, name: "Priority parking", weight: 0.079852, equal: 0.090909, delta: -0.011057 },
      { rank: 7, name: "Cognitive / fatigue support tools", weight: 0.074978, equal: 0.090909, delta: -0.015931 },
      { rank: 8, name: "Policy accommodations (e.g., dress code flexibility, headphone use)", weight: 0.070501, equal: 0.090909, delta: -0.020408 },
      { rank: 9, name: "Remote work capability", weight: 0.064075, equal: 0.090909, delta: -0.026834 },
      { rank: 10, name: "Transportation reimbursement", weight: 0.059231, equal: 0.090909, delta: -0.031678 },
      { rank: 11, name: "Physical workspace modifications", weight: 0.057672, equal: 0.090909, delta: -0.033237 },
    ]
  },
  6: {
    ...
    items: [
      { rank: 1, name: "Employee peer support groups (internal employees with shared experi...", weight: 0.193109, equal: 0.083333, delta: 0.109775 },
      { rank: 2, name: "Stigma-reduction initiatives", weight: 0.130256, equal: 0.083333, delta: 0.046922 },
      { rank: 3, name: "Anonymous benefits navigation tool or website (no login required)", weight: 0.090308, equal: 0.083333, delta: 0.006975 },
      { rank: 4, name: "Specialized emotional counseling", weight: 0.079909, equal: 0.083333, delta: -0.003424 },
      { rank: 5, name: "Inclusive communication guidelines", weight: 0.071169, equal: 0.083333, delta: -0.012164 },
      { rank: 6, name: "Manager training on handling sensitive health information", weight: 0.070972, equal: 0.083333, delta: -0.012361 },
      { rank: 7, name: "Professional-led support groups (external facilitator/counselor)", weight: 0.066748, equal: 0.083333, delta: -0.016585 },
      { rank: 8, name: "Written anti-retaliation policies for health disclosures", weight: 0.065130, equal: 0.083333, delta: -0.018204 },
      { rank: 9, name: "Strong anti-discrimination policies specific to health conditions", weight: 0.063864, equal: 0.083333, delta: -0.019470 },
      { rank: 10, name: "Clear process for confidential health disclosures", weight: 0.059082, equal: 0.083333, delta: -0.024251 },
      { rank: 11, name: "Confidential HR channel for health benefits, policies and insurance...", weight: 0.057287, equal: 0.083333, delta: -0.026046 },
      { rank: 12, name: "Optional open health dialogue forums", weight: 0.052165, equal: 0.083333, delta: -0.031168 },
    ]
  },
  7: {
  ...
  8: {
    ...
    items: [
      { rank: 1, name: "Flexibility for medical setbacks", weight: 0.154687, equal: 0.083333, delta: 0.071354 },
      { rank: 2, name: "Manager training on supporting team members during treatment/return", weight: 0.124079, equal: 0.083333, delta: 0.040746 },
      { rank: 3, name: "Long-term success tracking", weight: 0.102334, equal: 0.083333, delta: 0.019000 },
      { rank: 4, name: "Workload adjustments during treatment", weight: 0.089721, equal: 0.083333, delta: 0.006388 },
      { rank: 5, name: "Access to occupational therapy/vocational rehabilitation", weight: 0.081946, equal: 0.083333, delta: -0.001388 },
      { rank: 6, name: "Structured progress reviews", weight: 0.072191, equal: 0.083333, delta: -0.011142 },
      { rank: 7, name: "Buddy/mentor pairing for support", weight: 0.069346, equal: 0.083333, delta: -0.013988 },
      { rank: 8, name: "Flexible work arrangements during treatment", weight: 0.068431, equal: 0.083333, delta: -0.014902 },
      { rank: 9, name: "Online peer support forums", weight: 0.068325, equal: 0.083333, delta: -0.015008 },
      { rank: 10, name: "Phased return-to-work plans", weight: 0.059020, equal: 0.083333, delta: -0.024314 },
      { rank: 11, name: "Contingency planning for treatment schedules", weight: 0.056143, equal: 0.083333, delta: -0.027190 },
      { rank: 12, name: "Access to specialized work resumption professionals", weight: 0.053777, equal: 0.083333, delta: -0.029556 },
    ]
  },
  9: {
    ...
    items: [
      { rank: 1, name: "Executive sponsors communicate regularly about workplace support pr...", weight: 0.177838, equal: 0.083333, delta: 0.094505 },
      { rank: 2, name: "ESG/CSR reporting inclusion", weight: 0.122962, equal: 0.083333, delta: 0.039629 },
      { rank: 3, name: "Public success story celebrations", weight: 0.101589, equal: 0.083333, delta: 0.018256 },
      { rank: 4, name: "Executive-led town halls focused on health benefits and employee su...", weight: 0.078106, equal: 0.083333, delta: -0.005227 },
      { rank: 5, name: "Year-over-year budget growth", weight: 0.077801, equal: 0.083333, delta: -0.005532 },
      { rank: 6, name: "Support programs included in investor/stakeholder communications", weight: 0.076953, equal: 0.083333, delta: -0.006380 },
      { rank: 7, name: "Compensation tied to support outcomes", weight: 0.067727, equal: 0.083333, delta: -0.015606 },
      { rank: 8, name: "C-suite executive serves as program champion/sponsor", weight: 0.063951, equal: 0.083333, delta: -0.019383 },
      { rank: 9, name: "Cross-functional executive steering committee for workplace support...", weight: 0.060213, equal: 0.083333, delta: -0.023121 },
      { rank: 10, name: "Support metrics included in annual report/sustainability reporting", weight: 0.058149, equal: 0.083333, delta: -0.025185 },
      { rank: 11, name: "Executive accountability metrics", weight: 0.057550, equal: 0.083333, delta: -0.025784 },
      { rank: 12, name: "Dedicated budget allocation for serious illness support programs", weight: 0.057162, equal: 0.083333, delta: -0.026172 },
    ]
  },
  10: {
    ...
    items: [
      { rank: 1, name: "Practical support for managing caregiving and work", weight: 0.114708, equal: 0.050000, delta: 0.064708 },
      ...
      { rank: 11, name: "Emergency dependent care when regular arrangements unavailable", weight: 0.042999, equal: 0.050000, delta: -0.007001 },
      { rank: 12, name: "Unpaid leave job protection beyond local / legal requirements", weight: 0.042978, equal: 0.050000, delta: -0.007022 },
      { rank: 13, name: "Legal/financial planning assistance for caregivers", weight: 0.041332, equal: 0.050000, delta: -0.008668 },
      { rank: 14, name: "Mental health support specifically for caregivers", weight: 0.041290, equal: 0.050000, delta: -0.008710 },
      { rank: 15, name: "Manager training for supervising caregivers", weight: 0.039363, equal: 0.050000, delta: -0.010637 },
      { rank: 16, name: "Dependent care account matching/contributions", weight: 0.038215, equal: 0.050000, delta: -0.011785 },
      { rank: 17, name: "Caregiver peer support groups", weight: 0.037140, equal: 0.050000, delta: -0.012860 },
      { rank: 18, name: "Dependent care subsidies", weight: 0.034974, equal: 0.050000, delta: -0.015026 },
      { rank: 19, name: "Emergency caregiver funds", weight: 0.034931, equal: 0.050000, delta: -0.015069 },
      { rank: 20, name: "Modified job duties during peak caregiving periods", weight: 0.034619, equal: 0.050000, delta: -0.015381 },
    ]
  },
  11: {
    ...
    items: [
      { rank: 1, name: "Legal protections beyond requirements", weight: 0.115400, equal: 0.076923, delta: 0.038477 },
      { rank: 2, name: "Individual health assessments (online or in-person)", weight: 0.112016, equal: 0.076923, delta: 0.035093 },
      { rank: 3, name: "Policies to support immuno-compromised colleagues (e.g., mask proto...", weight: 0.104588, equal: 0.076923, delta: 0.027665 },
      { rank: 4, name: "Genetic screening/counseling", weight: 0.096957, equal: 0.076923, delta: 0.020034 },
      { rank: 5, name: "At least 70% coverage for regionally / locally recommended screenings", weight: 0.072063, equal: 0.076923, delta: -0.004860 },
      { rank: 6, name: "Full or partial coverage for annual health screenings/checkups", weight: 0.070034, equal: 0.076923, delta: -0.006889 },
      { rank: 7, name: "On-site vaccinations", weight: 0.069652, equal: 0.076923, delta: -0.007271 },
      { rank: 8, name: "Risk factor tracking/reporting", weight: 0.066010, equal: 0.076923, delta: -0.010913 },
      { rank: 9, name: "Regular health education sessions", weight: 0.063478, equal: 0.076923, delta: -0.013445 },
      { rank: 10, name: "Targeted risk-reduction programs", weight: 0.061622, equal: 0.076923, delta: -0.015301 },
      { rank: 11, name: "Paid time off for preventive care appointments", weight: 0.061405, equal: 0.076923, delta: -0.015518 },
      { rank: 12, name: "Workplace safety assessments to minimize health risks", weight: 0.054855, equal: 0.076923, delta: -0.022068 },
      { rank: 13, name: "Lifestyle coaching programs", weight: 0.051920, equal: 0.076923, delta: -0.025003 },
    ]
  },
  12: {
    ...
    items: [
      { rank: 1, name: "Regular program enhancements", weight: 0.200034, equal: 0.111111, delta: 0.088923 },
      { rank: 2, name: "Employee confidence in employer support", weight: 0.145168, equal: 0.111111, delta: 0.034057 },
      { rank: 3, name: "Innovation pilots", weight: 0.111299, equal: 0.111111, delta: 0.000188 },
      { rank: 4, name: "External benchmarking", weight: 0.111196, equal: 0.111111, delta: 0.000085 },
      { rank: 5, name: "Return-to-work success metrics", weight: 0.101878, equal: 0.111111, delta: -0.009233 },
      { rank: 6, name: "Program utilization analytics", weight: 0.100793, equal: 0.111111, delta: -0.010318 },
      { rank: 7, name: "Measure screening campaign ROI (e.g. participation rates, inquiries...", weight: 0.077227, equal: 0.111111, delta: -0.033884 },
      { rank: 8, name: "Business impact/ROI assessment", weight: 0.076737, equal: 0.111111, delta: -0.034374 },
      { rank: 9, name: "Employee satisfaction tracking", weight: 0.075667, equal: 0.111111, delta: -0.035444 },
    ]
  },
  13: {
    ...
    items: [
      { rank: 1, name: "Family/caregiver communication inclusion", weight: 0.183852, equal: 0.1, delta: 0.083852 },
      { rank: 2, name: "Employee testimonials/success stories", weight: 0.125551, equal: 0.1, delta: 0.025551 },
      { rank: 3, name: "Proactive communication at point of diagnosis disclosure", weight: 0.124916, equal: 0.1, delta: 0.024916 },
      { rank: 4, name: "Multi-channel communication strategy", weight: 0.114740, equal: 0.1, delta: 0.014740 },
      { rank: 5, name: "Ability to access program information and resources anonymously (me...", weight: 0.112105, equal: 0.1, delta: 0.012105 },
      { rank: 6, name: "Dedicated program website or portal", weight: 0.080913, equal: 0.1, delta: -0.019087 },
      { rank: 7, name: "New hire orientation coverage", weight: 0.066129, equal: 0.1, delta: -0.033871 },
      { rank: 8, name: "Regular company-wide awareness campaigns (at least quarterly)", weight: 0.064832, equal: 0.1, delta: -0.035168 },
      { rank: 9, name: "Manager toolkit for cascade communications", weight: 0.064606, equal: 0.1, delta: -0.035394 },
      { rank: 10, name: "Cancer awareness month campaigns with resources", weight: 0.062357, equal: 0.1, delta: -0.037643 },
    ]
  },
};
...
export default function ElementWeightingPage() {
  ...
  const combinedBenchmark = useMemo(() => {
    ...
    const dims: Record<number, { oldEq: number; newEq: number; wt: number; unsure: number; combined: number }> = {};
    for (let i = 1; i <= 13; i++) {
      const oldEqs = filteredCombined.map(c => c.dims[i]?.oldEq || 0);
      const newEqs = filteredCombined.map(c => c.dims[i]?.newEq || 0);
      const wts = filteredCombined.map(c => c.dims[i]?.wt || 0);
      const uns = filteredCombined.map(c => c.dims[i]?.unsure || 0);
      const cbs = filteredCombined.map(c => c.dims[i]?.combined || 0);
      dims[i] = {
        oldEq: Math.round(oldEqs.reduce((a, b) => a + b, 0) / oldEqs.length),
        newEq: Math.round(newEqs.reduce((a, b) => a + b, 0) / newEqs.length),
        wt: Math.round(wts.reduce((a, b) => a + b, 0) / wts.length),
        unsure: Math.round(uns.reduce((a, b) => a + b, 0) / uns.length),
        combined: Math.round(cbs.reduce((a, b) => a + b, 0) / cbs.length),
      };
    }
    const oldEqC = Math.round(filteredCombined.reduce((s, c) => s + c.oldEqComposite, 0) / filteredCombined.length);
    const newEqC = Math.round(filteredCombined.reduce((s, c) => s + c.newEqComposite, 0) / filteredCombined.length);
    ...
    return { dims, oldEqC, newEqC, wtC, unC, cbC };
  }, [filteredCombined]);
            ...
            {loading ? (
              ...
              <>
                ...
                <div className="flex items-center gap-8">
                  ...
                  <div className="ml-auto flex items-center gap-6">
                    <div>
                      <span className="text-2xl font-bold text-violet-700">{filteredCombined.filter(c => c.combinedComposite > c.newEqComposite).length}</span>
                      <span className="text-sm text-slate-600 ml-1">score higher</span>
                    </div>
                    <div>
                      <span className="text-2xl font-bold text-slate-800">
                        {(filteredCombined.reduce((s, c) => s + Math.abs(c.combinedComposite - c.newEqComposite), 0) / filteredCombined.length).toFixed(1)}
                      </span>
                      <span className="text-sm text-slate-600 ml-1">avg shift (pts)</span>
                    </div>
                    <div>
                  ...
                  >
                    <table className="w-full text-sm border-collapse">
                      ...
                      <tbody>
                        ...
                        <tr className="border-b border-slate-200">
                          <td className="sticky left-0 z-10 bg-white px-3 py-3 text-slate-800 font-semibold text-sm border-r border-slate-100">Old Dim Weights</td>
                          <td className="px-2 py-3 text-center font-bold text-slate-800 text-sm bg-slate-50 border-r border-slate-100">{combinedBenchmark?.oldEqC}</td>
                          {filteredCombined.map((c, i) => (
                            <td key={c.surveyId} className={`px-1 py-3 text-center font-semibold text-slate-800 ${i % 2 === 0 ? 'bg-slate-50/50' : ''}`}>{c.oldEqComposite}</td>
                          ))}
                        </tr>
                        ...
                        <tr className="border-b border-slate-200 bg-sky-50">
                          <td className="sticky left-0 z-10 bg-sky-50 px-3 py-3 text-sky-800 font-bold text-sm border-r border-sky-100">New Dim Weights</td>
                          <td className="px-2 py-3 text-center font-bold text-sky-800 text-sm bg-sky-100 border-r border-sky-100">{combinedBenchmark?.newEqC}</td>
                          {filteredCombined.map((c, i) => (
                            <td key={c.surveyId} className={`px-1 py-3 text-center font-bold text-sky-800 ${i % 2 === 0 ? 'bg-sky-50' : 'bg-sky-50/50'}`}>{c.newEqComposite}</td>
                          ))}
                        </tr>
                        ...
                        {/* Delta: Combined vs New Dim Weights */}
                        <tr className="border-b-2 border-slate-400 bg-slate-100">
                          <td className="sticky left-0 z-10 bg-slate-100 px-3 py-1.5 text-slate-600 text-xs font-bold border-r border-slate-200">&Delta; vs New</td>
                          <td className="px-2 py-1.5 text-center text-xs font-bold bg-slate-100 border-r border-slate-200">
                            {combinedBenchmark && <span className={(combinedBenchmark.cbC - combinedBenchmark.newEqC) >= 0 ? 'text-violet-700' : 'text-red-700'}>{(combinedBenchmark.cbC - combinedBenchmark.newEqC) >= 0 ? '+' : ''}{combinedBenchmark.cbC - combinedBenchmark.newEqC}</span>}
                          </td>
                          {filteredCombined.map((c) => {
                            const d = c.combinedComposite - c.newEqComposite;
                            return (
                              <td key={c.surveyId} className="px-1 py-1.5 text-center text-xs font-bold">
                        ...
                        {DIMENSION_ORDER.map((dim, idx) => (
                          <React.Fragment key={dim}>
                            ...
                            <tr className="border-b border-slate-100">
                              <td className="sticky left-0 z-10 bg-white px-3 py-1 text-slate-600 pl-4 text-xs font-medium border-r border-slate-100">Old Dim Wt</td>
                              <td className="px-2 py-2 text-center text-slate-700 text-sm bg-slate-50/50 border-r border-slate-100">{combinedBenchmark?.dims[dim]?.oldEq}</td>
                              {filteredCombined.map((c, i) => (
                                <td key={c.surveyId} className={`px-1 py-2 text-center text-slate-700 text-sm ${i % 2 === 0 ? 'bg-slate-50/30' : ''}`}>{c.dims[dim]?.oldEq}</td>
                              ))}
                            </tr>
                            ...
                            <tr className="border-b border-slate-100 bg-sky-50/30">
                              <td className="sticky left-0 z-10 bg-sky-50/30 px-3 py-1 text-sky-800 font-semibold pl-4 text-xs border-r border-sky-100/50">New Dim Wt</td>
                              <td className="px-2 py-2 text-center font-semibold text-xs bg-sky-100/30 border-r border-sky-100/50" style={{ color: getScoreColor(combinedBenchmark?.dims[dim]?.newEq || 0) }}>{combinedBenchmark?.dims[dim]?.newEq}</td>
                              {filteredCombined.map((c, i) => (
                                <td key={c.surveyId} className={`px-1 py-2 text-center text-xs font-semibold ${i % 2 === 0 ? 'bg-sky-50/40' : 'bg-sky-50/20'}`} style={{ color: getScoreColor(c.dims[dim]?.newEq || 0) }}>
                                  {c.dims[dim]?.newEq}
                                </td>
                            ...
                            <tr className="border-b border-slate-200 bg-slate-50/50">
                              <td className="sticky left-0 z-10 bg-slate-50/50 px-3 py-1 text-slate-500 pl-4 text-[10px] font-bold border-r border-slate-100">&Delta;</td>
                              <td className="px-2 py-1 text-center text-[10px] font-bold bg-slate-50/50 border-r border-slate-100">
                                {combinedBenchmark?.dims[dim]?.newEq != null && combinedBenchmark?.dims[dim]?.combined != null && (
                                  <span className={(combinedBenchmark.dims[dim].combined - combinedBenchmark.dims[dim].newEq) >= 0 ? 'text-violet-600' : 'text-red-600'}>
                                    {(combinedBenchmark.dims[dim].combined - combinedBenchmark.dims[dim].newEq) >= 0 ? '+' : ''}{combinedBenchmark.dims[dim].combined - combinedBenchmark.dims[dim].newEq}
                                  </span>
                                )}
                              </td>
                              {filteredCombined.map((c) => {
