    ...
    <>
      <div className="min-h-screen" style={{ backgroundColor: BRAND.gray[100] }}>
        ...
        <main id="profile-content" className="max-w-7xl mx-auto px-6 py-8">
          ...
          {/* ASSESSMENT SCORING SECTION - INLINE */}
          {(() => {
            // Calculate dimension scores using the helper functions
            ...
            // Aggregate breakdown
            let totalCurrentlyOffer = 0, totalPlanningScore = 0, totalAssessing = 0, totalNotAble = 0, totalUnsureScore = 0;
            for (let i = 1; i <= 13; i++) {
              const score = dimensionScores[i];
              totalCurrentlyOffer += score.breakdown.currentlyOffer;
              totalPlanningScore += score.breakdown.planning;
              totalAssessing += score.breakdown.assessing;
              totalNotAble += score.breakdown.notAble;
              totalUnsureScore += score.breakdown.unsure;
            }
            
            const tier = completedDimCount > 0 ? getPerformanceTier(weightedScore, isProvisional) : null;
            const hasAnyDimensionData = completedDimCount > 0;
            
            if (!hasAnyDimensionData) return null;
            
            return (
              <section className="mb-8 bg-indigo-50 rounded-xl p-6 border border-indigo-200">
                ...
                <div className="grid grid-cols-3 gap-4 mb-4">
                  <div className="bg-white rounded-lg p-3 border border-indigo-200">
                    ...
                    <p className="text-2xl font-bold" style={{ color: getScoreColor(weightedScore) }}>
                      {completedDimCount > 0 ? weightedScore : '—'}
                    </p>
                  </div>
                  <div className="bg-white rounded-lg p-3 border border-indigo-200">
                    <p className="text-xs text-gray-500 uppercase">Unweighted Avg</p>
                    <p className="text-2xl font-bold text-gray-700">
                      {completedDimCount > 0 ? unweightedScore : '—'}
                    </p>
                  </div>
                  <div className="bg-white rounded-lg p-3 border border-indigo-200">
                    <p className="text-xs text-gray-500 uppercase">Dimensions</p>
                    <p className="text-2xl font-bold text-gray-700">{completedDimCount}/13</p>
                  </div>
                </div>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                        ...
                        return (
                          <tr key={dim} className={`border-b ${isHighUnsure ? 'bg-amber-50' : 'hover:bg-gray-50'}`}>
                            <td className="px-3 py-2 font-medium text-gray-900">
                              D{dim}: {DIMENSION_NAMES_SCORING[dim]}
                            </td>
                      ...
                      <tr className="bg-indigo-100 font-bold border-t-2 border-indigo-300">
                        ...
                        <td className="px-2 py-3 text-center text-indigo-700">{unweightedScore}</td>
                        <td className="px-2 py-3 text-center text-indigo-600 text-xs">—</td>
                        <td className="px-2 py-3 text-center">
                          <span className="text-xl" style={{ color: getScoreColor(weightedScore) }}>{weightedScore}</span>
                        </td>
                        <td className="px-3 py-3">
                          <div className="flex items-center gap-2 text-xs">
                            <span className="text-green-600">✓{totalCurrentlyOffer}</span>
                            <span className="text-blue-600">●{totalPlanningScore}</span>
                            <span className="text-orange-500">○{totalAssessing}</span>
                            <span className="text-red-500">✗{totalNotAble}</span>
                            {totalUnsureScore > 0 && <span className="text-gray-400">?{totalUnsureScore}</span>}
                          </div>
                        </td>
                        <td className="px-2 py-3 text-center">
                          {insufficientDataCount > 0 && <span className="text-amber-600 text-xs">{insufficientDataCount} dims</span>}
                        </td>
                      </tr>
                    </tbody>
                  </table>
