...
export default function GeneralBenefitsPage() {
  ...
  useEffect(() => {
    ...
    const savedStep = parseInt(localStorage.getItem('general_progress') || '0', 10)
    if (!isNaN(savedStep)) setStep(savedStep)
  }, [])

  useEffect(() => {
    localStorage.setItem('general_progress', step.toString())
  }, [step])

  const toggleSelected = (opt) => {
    setSelectedPrograms((prev) => (prev.includes(opt) ? prev.filter((x) => x !== opt) : [...prev, opt]))
  }
  const toggleFuture = (opt) => {
    setFuturePrograms((prev) => (prev.includes(opt) ? prev.filter((x) => x !== opt) : [...prev, opt]))
  }
  ...
  const handleSubmit = () => {
    ...
    localStorage.removeItem('general_progress')
  ...
  const leaveFlex = [
    'Paid family/medical leave beyond legal requirements','Flexible work schedules','Remote work options','Job sharing programs',
    'Phased retirement','Sabbatical programs','Dedicated caregiver leave (separate from family leave)',
  ]
  const wellnessSupport = [
    'Employee assistance program (EAP)','Physical wellness programs (fitness, nutrition, ergonomics)',
    'Mental wellness programs (stress management, mindfulness, resilience)','On-site health services',
    'Mental health resources (therapy, counseling)','Caregiving support resources',
    'Tailored support programs for employees managing cancer or other serious health conditions',
  ]
  const financialLegal = [
    'Financial counseling/planning','Student loan assistance','Identity theft protection',
    'Legal assistance/services (will preparation, family law, medical directives)',
  ]
  const careNav = [
    'Care coordination for complex conditions','Second opinion services or facilitation',
    'Specialized treatment center networks','Travel support for specialized care',
    'Clinical guidance and navigation','Medication access and affordability programs',
  ]
  const allPrograms = [...standard, ...leaveFlex, ...wellnessSupport, ...financialLegal, ...careNav]

  // Step definitions: CB1 categories
  const categorySteps = [
  ...
  // Determine if CB1a (multi-country) and CB2b are shown
  const showPercentStep = !isUSOnly && step === categorySteps.length
  const showFutureStep = step === categorySteps.length + (isUSOnly ? 0 : 1)

  // Save total steps for progress
  useEffect(() => {
    const totalSteps = categorySteps.length + (isUSOnly ? 1 : 2)
    localStorage.setItem('general_total', totalSteps.toString())
  }, [isUSOnly])
