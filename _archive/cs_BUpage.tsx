...
export default function SupportProgramsPage() {
  ...
  const [cb3b, setCb3b] = useState([])
  const [cb3c, setCb3c] = useState([])
  const [cb3d, setCb3d] = useState([])
  const [or1, setOr1] = useState('')
  const [or2a, setOr2a] = useState([])
  const [or2b, setOr2b] = useState('')
  const [or3, setOr3] = useState([])
  const [or5a, setOr5a] = useState([])
  ...
  useEffect(() => {
    ...
    const savedStep = parseInt(localStorage.getItem('current_progress') || '0', 10)
    if (!isNaN(savedStep)) setStep(savedStep)
  }, [])

  useEffect(() => {
    localStorage.setItem('current_progress', step.toString())
  }, [step])
  ...
  const handleSubmit = () => {
    localStorage.setItem(
      'current_data',
      JSON.stringify({
        cb3a, cb3b, cb3c, cb3d, or1, or2a, or2b, or3, or5a, or6,
        otherCb3b, otherCb3d, otherOr2a, otherOr3, otherOr5a, otherOr6,
        complete: true,
      })
    )
    localStorage.removeItem('current_progress')
  ...
  const stepsArray = [
    ...
    },
    {
      content: (
        <div className="space-y-4">
          ...
          <p className="text-sm text-gray-600 mb-3">(Select ONE)</p>
          {[
    ...
    },
    {
      content: (
        <div className="space-y-4">
          <h2 className="text-lg font-semibold text-gray-800">
            CB3c. Which serious health conditions does/will your program address?
          </h2>
          <p className="text-sm text-gray-600 mb-3">
            (Select ALL that apply. For multi-country organizations: Select any condition that is covered in at least one of your locations. Your program doesnâ€™t need to address all conditions in every location.)
          </p>
          {[
    ...
    },
    {
      content: (
        <div className="space-y-4">
          <h2 className="text-lg font-semibold text-gray-800">
            CB3d. How were/will the workplace support programs for employees managing cancer or other serious health conditions be primarily developed?
          </h2>
          <p className="text-sm text-gray-600 mb-3">(Select ALL that apply)</p>
          {[
    ...
    },
    {
      content: (
        <div className="space-y-4">
          <h2 className="text-lg font-semibold text-gray-800">
            OR2a. What triggered your organization to develop support beyond basic legal requirements?
          </h2>
          <p className="text-sm text-gray-600 mb-3">(Select ALL that apply)</p>
          {[
    ...
    },
    {
      content: (
        <div className="space-y-4">
          <h2 className="text-lg font-semibold text-gray-800">
            OR3. What are the primary barriers preventing more comprehensive support for employees managing cancer or other serious health conditions?
          </h2>
          <p className="text-sm text-gray-600 mb-3">(Select ALL that apply)</p>
          {[
      ...
      content: (
        <div className="space-y-4">
          ...
          {[
            ...
            'Emergency dependent care when regular arrangements unavailable','Legal/financial planning resources',
            'Some other support (specify)','Not able to provide caregiver support at this time',
          ].map((opt) => (
  ...
  // Save total steps for dynamic progress
  useEffect(() => {
    localStorage.setItem('current_total', stepsArray.length.toString())
  }, [])
