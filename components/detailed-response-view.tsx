...
const sectionNames: Record<string, string> = {
  ...
  dimension4_complete: 'Cancer Support Resources',
...
function getPerformanceTier(score: number, isProvisional: boolean): { name: string; color: string; bg: string; isProvisional: boolean } {
  ...
  if (score >= 80) {
    tier = { name: 'Leading', color: '#1B5E20', bg: '#E8F5E9' }
  } else if (score >= 64) {
    tier = { name: 'Established', color: '#0D47A1', bg: '#E3F2FD' }
  } else if (score >= 50) {
    tier = { name: 'Progressing', color: '#E65100', bg: '#FFF8E1' }
  } else {
    tier = { name: 'Building', color: '#37474F', bg: '#ECEFF1' }
  }
